...
public class HtmlAdapter {
    ...
    public void beforeParseColumnName(Elements columnNameElements, CsvResultSet resultSet) {
        resultSet.addColumn(getNameMappingValue("股票代码"));
        ...
        for (Element columnNameElement : rowSpan) {
            ...
            resultSet.addColumn(getNameMappingValue(nameInPage));
        }
        ...
        for (int i = 0; i < categorys.size(); i++) {
            ...
            resultSet.addColumn(getNameMappingValue(prefix + CommUtils.trim(other.get(otherIndex++).text())));
            resultSet.addColumn(getNameMappingValue(prefix + CommUtils.trim(other.get(otherIndex++).text())));
        }
    }

    private String getNameMappingValue(String nameInPage) {
    ...
    public String parseCellValue(Element cellElement) {
        ...
        if (indexOf1E4 != -1) {
            return String.valueOf(new BigDecimal(str.substring(0, indexOf1E4)).multiply(new BigDecimal(10000)));
        }
        int indexOf1E8 = str.lastIndexOf('亿');
        if (indexOf1E8 != -1) {
            return String.valueOf(new BigDecimal(str.substring(0, indexOf1E8)).multiply(new BigDecimal(100000000)));
        }
