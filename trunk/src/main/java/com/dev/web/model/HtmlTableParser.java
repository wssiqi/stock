...
public class HtmlTableParser {
    ...
    public static void main(String[] args) {
        ...
        for (File htmlFile : pageFilesDir.listFiles()) {
            String fileExtension = CommonUtils.getFileExtension(htmlFile);
            if (!"html".equalsIgnoreCase(fileExtension)) {
                continue;
            }
            HtmlTableParser parser = new HtmlTableParser(new HtmlAdapter());
            ResultSet resultSet = parser.getResultSet(htmlFile);
            String nameWithoutExtension = CommonUtils.getFileNameWithoutExtension(htmlFile);
            File outCsvFile = new File(csvFilesDir, nameWithoutExtension + ".csv");
            System.out.println(htmlFile);
            try {
                new Csv().write(CommonUtils.getAbsolutePath(outCsvFile), resultSet, "gbk");
    ...
    public ResultSet getResultSet(File htmlFile) {
        ...
        try {
            Document document = Jsoup.parse(htmlFile, htmlAdapter.getHtmlFileCharset().name());
            ...
            throw new StockException(String.format("Parse file '%s' failed.", htmlFile.getAbsolutePath()), e);
        }
    }

    private void parseColumnNames(CsvResultSet resultSet, Document document) {
        Elements columnNameElements = document.select(htmlAdapter.getColumnNamesCssQuery());
